...
class SuperlistaSupabaseUser extends BaseAuthUser {
  SuperlistaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> superlistaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SuperlistaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
